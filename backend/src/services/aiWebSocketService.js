...
class AIWebSocketService {
    ...
    this.io.on('connection', (socket) => {
      ...
      socket.on('authenticate', (data) => {
        ...
        if (client) {
          ...
          console.log(`AI Chat client ${socket.id} authenticated for user ${data.userId}`);
        }
      });
  ...
  async handleChatMessage(socket, data) {
    try {
      ...
      let session = null;
      if (sessionId) {
        session = await AISession.findOne({ _id: sessionId, userId: client.userId });
      }
  ...
  async handleMoodAnalysis(socket, data) {
    try {
      const client = this.connectedClients.get(socket.id);
      if (!client || !client.userId) {
        socket.emit('mood-analysis-error', { message: 'Authentication required' });
        return;
      }
      ...
      const session = await AISession.findOne({ _id: sessionId, userId: client.userId });
      if (!session) {
        socket.emit('mood-analysis-error', { message: 'Conversation not found' });
        return;
      }
  ...
  async handleWellnessSuggestions(socket, data) {
    try {
      const client = this.connectedClients.get(socket.id);
      if (!client || !client.userId) {
        socket.emit('wellness-suggestions-error', { message: 'Authentication required' });
        return;
      }
      ...
      const session = await AISession.findOne({ _id: sessionId, userId: client.userId });
      if (!session) {
        socket.emit('wellness-suggestions-error', { message: 'Conversation not found' });
        return;
      }
  ...
  broadcastToSession(sessionId, event, data) {
    ...
    const sessionClients = Array.from(this.connectedClients.values())
